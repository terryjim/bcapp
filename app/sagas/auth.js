...
import {apiUrlV2, appKey, appSecret} from '../config'
...
export function * login(payload) {
  try {
    ...
    const ret = yield call(request, apiUrlV2.login, options, extra)
...
export function * register(payload) {
  try {
    ...
    //options.pwd = crypto.MD5(options.pwd).toString()
    const ret = yield call(request, apiUrlV2.appRegister, options, extra)
    //const {userUuid,header_address,userName,token} = ret
...
export function * resetPassword(payload) {
  try {
    ...
    const ret = yield call(request, apiUrlV2.resetPassword, options, extra)
    Tips.toast("操作成功")
